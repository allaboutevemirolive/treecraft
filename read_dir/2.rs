...
fn retrieve_metada(dir_path: &str) -> io::Result<Vec<FileInfo>> {
    ...
    for entry_result in fs::read_dir(dir_path)? {
        ...
        if file_name == "." || file_name == ".." || FileInfo::is_hidden(&file_name.to_string_lossy()) {
            continue;
        }

        let metadata = fs::symlink_metadata(&path)?;
        let file_info = FileInfo::new(&file_name.to_string_lossy(), &metadata);
...
fn main() -> io::Result<()> {
    ...
    let entries = retrieve_metada(directory_path)?;
